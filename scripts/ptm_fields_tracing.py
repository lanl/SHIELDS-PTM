"""
Routines to trace magnetic field lines, determine the location of minimum B along a magnetic field line,
and to determine the equatorial location of a minimum B location for a given MLT and radial distance.
...
from numpy import fromfile,pi,cos,sin,zeros,zeros_like,r_,abs
...
def get_B_fields(istep):
    ...
    bx=fromfile('bx3d_%4.4i.bin' % istep).reshape((nx,ny,nz))
    by=fromfile('by3d_%4.4i.bin' % istep).reshape((nx,ny,nz))
    bz=fromfile('bz3d_%4.4i.bin' % istep).reshape((nx,ny,nz))
...
def trace_field_line(mlt,rad,lat,ds=0.01,istep_max=100000):
    ...
    magnetic field line to both ionospheres
