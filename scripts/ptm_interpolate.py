...
def gauss_interp(xwant,ywant,zwant,fdict,key,smoothingDegree=0.5,numNeighbors=24):
  ...
  try:
    ...
    if(not fdict.has_key('z')): missingKeys+='z'
    raise Exception('Error in gaussian_interp: fdict does not have expected keys: {'+missingKeys.strip()+'}')

  # Check that data file contains the correct components

  if(not fdict.has_key(key)): raise Exception('Error in gauss_interp: requested key {'+key+'} was not present in fdict')
